...
export default class QuadTreeManager extends cc.Component {
  ...
  protected createNode (): void {
    const random = (min: number, max: number): number => min + Math.floor((max - min) * Math.random());
    ...
    this.objects.push(new Rect<CustomData>({
      x: node.x,
      y: node.y,
      width,
      height,
  ...
  protected update (dt: number): void {
    ...
    for (let i = 0; i < length; ++i) {
      ...
      if (this.isTree) {
        ...
        const r = objs.some(o => cc.Intersection.rectRect(
